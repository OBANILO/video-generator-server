...
def get_italic_font():
    for path in ['/usr/share/fonts/truetype/ubuntu/Ubuntu-BI.ttf','/usr/share/fonts/truetype/ubuntu/Ubuntu-LI.ttf','/usr/share/fonts/truetype/liberation/LiberationSans-BoldItalic.ttf','/usr/share/fonts/truetype/dejavu/DejaVuSans-BoldOblique.ttf']:
        if os.path.exists(path): return path
...
def build_karaoke_filter(segments, font):
    ...
    for seg in segments:
        ...
        if len(lines)==1:
            parts.append(f"drawtext=fontfile={font}:text='{ffmpeg_escape(lines[0])}':"
            ...
            for li,line in enumerate(lines):
                parts.append(f"drawtext=fontfile={font}:text='{ffmpeg_escape(line)}':"
                             f"fontsize={FONT_SIZE}:fontcolor=white@1.0:"
                             f"borderw=4:bordercolor=black@1.0:"
                             f"shadowcolor=black@0.95:shadowx=3:shadowy=3:"
...
def build_ffmpeg_command(image_path, audio_path, output_path, duration, fps, font, font_italic, lyrics_segments=None, artist_name="SORLUNE"):
...
def generate_video_job(job_id, image_path, audio_path, output_path, lyrics_segments=None, artist_name="SORLUNE"):
    try:
        ...
        duration=get_audio_duration(audio_path); font=get_best_font(); font_italic=get_italic_font()
        cmd=build_ffmpeg_command(image_path,audio_path,output_path,duration,25,font,font_italic,lyrics_segments=lyrics_segments,artist_name=artist_name)
...
def generate_video():
    ...
    audio_url=data.get('audio_url'); image_url=data.get('image_url'); api_key=data.get('api_key','default')
    lyrics_text=data.get('lyrics','').strip(); openai_key=data.get('openai_key','').strip(); artist_name=data.get('artist','SORLUNE').strip()
    if not audio_url or not image_url: return jsonify({'error':'Missing audio_url or image_url'}),400
    ...
    return jsonify({'status':'started','job_id':job_id,'lyrics_mode':'whisper' if openai_key else 'fallback'}),200
...
def check_status(api_key):
    ...
    if job['status']=='completed': response['video_url']=request.host_url.rstrip('/')+f'/videos/{api_key}/{api_key}.mp4'
...
def clear_cache():
    data=request.get_json(); api_key=data.get('api_key') if data else None
    if api_key:
...
def process_audio():
    ...
    except: return jsonify({'error':'Could not read audio duration'}),500
